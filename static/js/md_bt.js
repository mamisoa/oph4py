...
function responseHandler_wlCodes(res) { // used if data-response-handler="responseHandler_wlCodes"
    ...
    $.each(list, function (i) {
        display.push({
            ...
            'priceTag': checkIfNull(((Math.round(JSON.parse(list[i]['nomenclature']['price_list'])[0]*list[i]['nomenclature']['supplement_ratio'])*100)/100+(JSON.parse(list[i]['nomenclature']['price_list'])[3])),-1),
