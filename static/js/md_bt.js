...
// wlCodes table
function responseHandler_wlCodes(res) {
	// used if data-response-handler="responseHandler_wlCodes"
	let list = res.items;
	let display = [];
	$.each(list, function (i) {
		display.push({
			...
			priceList: checkIfNull(
				list[i]["nomenclature"]["price_list"],
				[-1, -1, -1, -1]
			),
			inami: checkIfNull(
				JSON.parse(list[i]["nomenclature"]["price_list"])[0],
				-1
			),
			i1600: checkIfNull(
				JSON.parse(list[i]["nomenclature"]["price_list"])[1],
				-1
			),
			i1300: checkIfNull(
				JSON.parse(list[i]["nomenclature"]["price_list"])[2],
				-1
			),
			priceTag: checkIfNull(
				(Math.round(
					JSON.parse(list[i]["nomenclature"]["price_list"])[0] *
						list[i]["nomenclature"]["supplement_ratio"]
				) *
					100) /
					100 +
					JSON.parse(list[i]["nomenclature"]["price_list"])[3],
				-1
			),
			supplementRatio: checkIfNull(
				list[i]["nomenclature"]["supplement_ratio"],
				SUPPLEMENT_RATIO
			),
...
function detailFormatter_wlCodes(index, row) {
	...
	html.push('<p class=""><span class="fw-bold">ID: </span>' + row.id);
	html.push(
		'<p class=""><span class="fw-bold">Datestamp: </span>' + row.date + "</p>"
	);
	html.push(
		'<p class=""><span class="fw-bold">Code description: </span>' +
			row.codeDescription +
			"</p>"
	);
	html.push(
		'<p class=""><span class="fw-bold">Supplement ratio: </span>' +
			row.suplementRatio +
			"</p>"
	);
	html.push(
		'<p class=""><span class="fw-bold">Age[min,max]: </span> [' +
			row.minAge +
			"," +
			row.maxAge +
			"] </p>"
	...
	html.push(
		'<p class=""><span class="fw-bold">Min recurrency: </span>' +
			row.minRecurrency +
			"</p>"
	);
	html.push(
		'<p class=""><span class="fw-bold">Add documents: </span>' +
			row.addDocuments +
			"</p>"
	);
	html.push(
...
function setCancelled(id, table, desc) {
	bootbox.confirm({
		...
		callback: function (result) {
			...
			dataStr = JSON.stringify(dataObj);
			if (result == true) {
				crudp(table, id, "PUT", dataStr).then((data) =>
...
function delWlCode(dataObj) {
	bootbox.confirm({
		centerVertical: true,
		message:
			"Are you sure you want to delete this code: " + dataObj.code + " ?",
		closeButton: false,
		buttons: {
			confirm: {
				label: "Yes",
		...
		callback: function (result) {
			if (result == true) {
				...
				function onTransactionSubstractUpdate() {
					...
					console.log(
						"filter ---> ",
						filterTransactionObject(newTransactionObj)
					);
					crudp(
						"transactions",
						id,
						"PUT",
						JSON.stringify(filterTransactionObject(newTransactionObj))
					)
						.then(async () => {
							const keysToRemove = [
								"created_on",
...
function operateFormatter_wlCodes(value, row, index) {
	...
	html.push(
		'<a class="remove ms-1" href="javascript:void(0)" title="Delete code"><i class="fas fa-trash-alt"></i></a>'
	);
