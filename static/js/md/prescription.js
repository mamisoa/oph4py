...
function initializePrescriptionObject() {
	// Check if required global variables are available
	if (typeof userObj === "undefined" || typeof usermdObj === "undefined") {
		console.warn(
			"userObj or usermdObj not available yet, deferring prescription initialization"
		);
		return false;
	}
...
if (!initializePrescriptionObject()) {
	$(document).ready(function () {
		// Try again when DOM is ready
		if (!initializePrescriptionObject()) {
			// If still not available, try after a short delay
			setTimeout(initializePrescriptionObject, 100);
		}
	});
}
...
$("#btnMxRx").click(function () {
	...
	getWlItemData(
		...
		wlId,
		"",
		"prescribed=False&@lookup=medic!:id_medic_ref"
	).then(function (data) {
		...
		if (data.count > 0 && data.status != "error") {
			...
			for (item of dataObj) {
				...
				let posology =
					...
					" " +
					checkIfDataIsNull(item["medic.form"], "unit(s)") +
					" " +
					item.frequency;
...
$("#mxRxModalPrint").click(function () {
	// get medications from current wl and not prescribed
	getWlItemData("mx", wlId, "", "&prescribed=False").then(function (data) {
		let dataObj = data.items;
		console.log(data);
		if (data.count > 0 && data.status != "error") {
							...
							body: [
								...
								[
									{
										...
										text: [
											...
											{
												text: prescObj["name"] + " " + prescObj["firstname"],
												fontSize: 8,
												bold: true,
											},
										],
								...
								[
									{
										...
										stack: [
											{
												text: "Réservé à la vignette du conditionnement",
												fontSize: 6,
											},
											{
												canvas: [
								...
								[
									...
									{},
									{
										...
										stack: [
											{
												text: "Date et signature du prescripteur",
												alignment: "center",
												fontSize: 6,
												margin: [0, 2, 0, 2],
											},
