...
$("#btnGxRx").click(function () {
	...
	for (item of dataObj) {
		...
		if (item["laterality"] == "right") {
			...
			html.push(
				'<button type="button" class="btn btn-primary btn-sm print-rx" onclick="rxDataButton(this.getAttribute(\'data-rx-obj\'),\'right\');" data-rx-obj=\'' +
					JSON.stringify(itemBtn) +
			...
			html.push("<td>"); // 4th col
			html.push(
				'<button type="button" class="btn btn-primary btn-sm print-rx" onclick="rxDataButton(this.getAttribute(\'data-rx-obj\'),\'left\');" data-rx-obj=\'' +
					JSON.stringify(itemBtn) +
					'\'><i class="fas fa-file-import"></i></button>'
			);
			html.push("</td>");
...
function globalRx2presc(rxObj) {
	...
	if (rxObj["tint"] == "") {
		rxObj["tint"] = "o";
	} else if (rxObj["tint"] == "False") {
		// non med tint
		rxObj["tint"] = "X";
		rxObj["tintnonmed"] = "X";
	} else if (rxObj["tint"] == "True") {
		// med tint
		rxObj["tint"] = "X";
		rxObj["tintmed"] = "X";
	} else {
	...
	if (rxObj["photo"] == "") {
		rxObj["photo"] = "o";
	} else if (rxObj["photo"] == "False") {
		// non med photo
		rxObj["photo"] = "X";
		rxObj["photononmed"] = "X";
	} else if (rxObj["photo"] == "True") {
		// med photo
		rxObj["photo"] = "X";
		rxObj["photomed"] = "X";
	} else {
	...
	if (rxObj["glass_type"] == "progressive") {
		...
		rxObj["multifocal"] = "X";
		rxObj["progressive"] = "X";
	} else if (rxObj["glass_type"] == "bifocal") {
		// non med photo
		rxObj["monofocal"] = "o";
		rxObj["multifocal"] = "X";
		rxObj["bifocal"] = "o";
	} else if (rxObj["glass_type"] == "degressive") {
		// med photo
		rxObj["monofocal"] = "o";
		rxObj["multifocal"] = "X";
		rxObj["degressive"] = "X";
...
$("#GxRxFormModal").submit(function (e) {
	...
	fetch(HOSTURL + "/" + APP_NAME + "/api/uuid", { method: "GET" })
		...
		.then((data) => {
				...
				content: [
						...
						table: {
							...
							body: [
								...
								[
									{ text: "" }, // spacer
									"",
									{ text: "TABO", bold: true, alignement: "right" },
									"",
								], // 4 columns
								[
									{ text: "" }, // spacer
					...
					}, // end of row
					{
						...
						text: [
							...
							{ text: "Note: " },
							{ text: finalRxObj["remarks"] + "\n", italics: true },
						],
					}, // end of row
					{
						canvas: [
							{ type: "line", x1: 0, y1: 10, x2: 515, y2: 10, lineWidth: 1 },
							{ type: "rect", x: 0, y: 12, w: 515, h: 2, color: "white" },
						], // spacer
					},
					{
						text: "EQUIPEMENT",
					...
					}, // end of row
					{
						...
						columns: [
							[
								{
									text: [
										{ text: finalRxObj["monofocal"] + " UNIFOCAL\n" },
								...
								},
								{
									...
									text: [
										{
											text: finalRxObj["tintnonmed"] + " Sans filtre médical\n",
										},
										{
											text:
												finalRxObj["tintmed"] +
												" Avec filtre médical(*) - Type ",
										},
										{ text: "", italics: true },
									],
								},
								...
								},
								{
									...
									text: [
										{
											text:
												finalRxObj["photononmed"] + " Sans filtre médical\n",
										},
										{
											text:
												finalRxObj["photomed"] +
							...
							[
								...
								},
								{
									margin: [15, 0, 10, 2],
									text: [
										{ text: "o" + " Avec coquille et ventouse\n" },
										{ text: "o" + " Avec micropores\n\n" },
									],
								},
								{
									text: [
										{ text: "o" + " FILTRE DE RYSER - Calibrage(densité): " },
										{ text: "", italics: true },
									],
								},
							], // right column
			...
			crudp("glasses_rx_list", "0", "POST", finalDbStr).then((data) =>
			...
			if (formObj.actionType === "email") {
				...
				pdf.getBase64((base64Data) => {
					...
					const emailData = {
						...
						content: content,
						attachmentName: `prescription_lunettes_${patientObj.last_name}_${patientObj.first_name}.pdf`,
					...
					fetch(HOSTURL + "/" + APP_NAME + "/api/email/send_with_attachment", {
						...
						.then((result) => {
							...
							if (result.status === "success") {
								...
								notifyUser(
									"Erreur lors de l'envoi: " + result.message,
									"danger"
								);
							}
						...
						.catch((error) => {
							...
							notifyUser("Erreur lors de l'envoi de l'email", "danger");
						});
				});
			} else {
