function responseHandler(res) {
	...
	$.each(list, function (i) {
		display.push({
			...
			rx_far:
				round2dec(list[i].sph_far) +
				"(" +
				round2dec(list[i].cyl_far) +
				"x" +
				list[i].axis_far +
				")",
			se_far: (
...
window.operateEvents = {
	"click .edit": function (e, value, row, index) {
		...
		$("#rxFormModal [name=timestamp]").val(
...
window.operateEvents_km = {
	"click .edit": function (e, value, row, index) {
		...
		$("#kmFormModal [name=timestamp]").val(
			row["timestamp"].split(" ").join("T")
		);
		$("#kmFormModal [name=id_worklist]").val(row.id_worklist);
...
function responseHandler_vx(res) {
	// used if data-response-handler="responseHandler_vl"
	...
	$.each(list, function (i) {
		if (list[i]["exam"] == "ark") {
			rx =
				round2qter(list[i]["sph5"], true) +
				"(" +
				round2qter(list[i]["cyl5"], true) +
				"x" +
				Math.round(list[i]["axis5"]) +
				"°)";
			kx = [
				round2dec(list[i]["k1"], false) + "x" + list[i]["k1_axis"] + "°",
...
function operateFormatter_vx(value, row, index) {
	let html = ['<div class="d-flex justify-content-between">'];
	html.push(
		'<a class="import" href="javascript:void(0)" title="Cache ' +
			row.side +
...
window.operateEvents_vx = {
	"click .import": function (e, value, row, index) {
		...
		if (row.exam == "wf") {
			$(domIdRx + " [name=sph_far]")
				.val(round2qter(row.sph5, true))
				.trigger("change");
			$(domIdRx + " [name=cyl_far]")
				.val(round2qter(row.cyl5, true))
				.trigger("change");
			$(domIdRx + " [name=axis_far]")
				.val(Math.round(row.axis5))
				.trigger("change");
			$(domIdRx + " [name=sph_int]")
				.val(round2qter(row.sph5, true))
				.trigger("change");
			$(domIdRx + " [name=cyl_int]")
				.val(round2qter(row.cyl5, true))
				.trigger("change");
			$(domIdRx + " [name=axis_int]")
				.val(Math.round(row.axis5))
				.trigger("change");
			$(domIdRx + " [name=sph_close]")
				.val(round2qter(row.sph5, true))
				.trigger("change");
			$(domIdRx + " [name=cyl_close]")
				.val(round2qter(row.cyl5, true))
				.trigger("change");
			$(domIdRx + " [name=axis_close]")
				.val(Math.round(row.axis5))
				.trigger("change");
		}
		if (row.exam == "ark") {
...
function responseHandler_cvrx(res) {
	...
	$.each(list, function (i) {
		display.push({
			...
			rxright:
				`${list[i]["SphR"]}(${list[i]["CylR"]}x${list[i]["AxisR"]}°) ` +
				`VA=${checkIfDataIsNull(list[i]["vaR"])}`,
			rxleft:
				`${list[i]["SphL"]}(${list[i]["CylL"]}x${list[i]["AxisL"]}°) ` +
				`VA=${checkIfDataIsNull(list[i]["vaL"])}`,
			pupilsd: `${list[i]["pdB"]}mm`,
		});
	});
	return { rows: display, total: list.count };
}
...
function operateFormatter_cvrx(value, row, index) {
	...
	html.push(
		'<a class="import" href="javascript:void(0)" title="cache ' +
			index +
			' km"><i class="fas fa-file-download"></i></a>'
	);
	html.push("</div>");
	return html.join("");
}
...
function rowStyle_rx(row) {
	let bg,
		statusColor = {
			"Objective Data": "white",
			"Full Correction": "white",
			Prescription: "#00FF00",
			"Current Spectacles": "papayawhip",
			"Last Prescription": "papayawhip",
		};
...
function removecvMeasure(mesType, id) {
	...
	exportDict[mesType]["measures"].forEach(function (measure, index) {
		...
		exportDict[mesType]["count"] = String(
			parseInt(exportDict[mesType]["count"], 10) - 1
		);
	});
}
...
function cacheCVtbl(data, datatype) {
	...
	if (datatype == "rx") {
		...
		exportDict["rx"]["count"] = String(
			1 + parseInt(exportDict["rx"]["count"], 10)
		);
	} else {
		...
		exportDict["machine"] = data["machine"];
		exportDict["km"]["count"] = String(
			1 + parseInt(exportDict["km"]["count"], 10)
		);
	}
	html.push(
		`<td><button type="button" onclick="removecvMeasure('${datatype}',${cvtblCounter});" class="btn btn-danger"><i class="fas fa-trash-alt"></i></button></td>`
	);
	html.push("</tr>");
	cvtblCounter += 1;
	...
	console.log(
		"item count:",
		document.getElementById("cvtblBody").childElementCount
	);
}
