...
for (let id of idRxArr) {
	...
	$(id + " .rxDiv input").change(function () {
		let SEf = (
			parseFloat($(id + " input[name=sph_far]").val()) +
			0.5 * parseFloat($(id + " input[name=cyl_far]").val())
		).toFixed(2);
		let SEi = (
			parseFloat($(id + " input[name=sph_int]").val()) +
			0.5 * parseFloat($(id + " input[name=cyl_int]").val())
		).toFixed(2);
		let SEc = (
			parseFloat($(id + " input[name=sph_close]").val()) +
			0.5 * parseFloat($(id + " input[name=cyl_close]").val())
		).toFixed(2);
		$(id + " .SEf").html(SEf);
		$(id + " .SEi").html(SEi);
		$(id + " .SEc").html(SEc);
		...
		if ($(this).attr("name") == "axis_far") {
			$(id + " input[name=axis_int]").val(
				$(id + " input[name=axis_far]").val()
			);
			$(id + " input[name=axis_close]").val(
				$(id + " input[name=axis_far]").val()
			);
		}
	});
...
for (let id of idKmArr) {
	for (let km of kmArr) {
		setCounter(id, km, 0.25, 35.0, 65.0, 2, false);
	}
	for (let axis of axisArr) {
		setCounter(id, axis, 5, 0, 180, 0, false);
	}
	...
	$(id + " input").change(function () {
		let k1d = parseFloat(
			diopter2mm(parseFloat($(id + " input[name=k1]").val()))
		);
		let k2d = parseFloat(
			diopter2mm(parseFloat($(id + " input[name=k2]").val()))
		);
		let kmd = (k1d + k2d) / 2;
		$(id + " .kmCalculated").html(
			kmd.toFixed(2) + "mm (" + diopter2mm(kmd) + "D)"
		);
...
function setCounter(id_count, count_class, step, min, max, precision, sign) {
	$(id_count + " .btn.counter_down_" + count_class).click(function () {
		...
		if (value >= min + step) {
			...
			sign == true
				? set > 0
					? (result = "+" + set.toFixed(precision))
					: (result = set.toFixed(precision))
				: (result = set.toFixed(precision));
			$(id_count + " input.counter_" + count_class)
				.val(result)
				.trigger("change");
		} else {
			set = max;
		...
		if (count_class == "add_close") {
			...
			$(id_count + " input.cyl_close").val(
				$(id_count + " input.cyl_far").val()
			);
			...
			$(id_count + " input.axis_int").val(
				$(id_count + " input.axis_far").val()
			);
			$(id_count + " input.axis_close").val(
				$(id_count + " input.axis_far").val()
			);
		}
	});
...
function eventSync(rx, display = "") {
	if (rx == "#idRightRx") {
		...
		$(
			'#idLeftRx input[name=rx_origin][value="' +
				$("#idRightRx input[name=rx_origin]:checked").val() +
				'"]'
		).prop("checked", true);
		$("#idLeftRx input[name=va_far]").val(
			$('#idRightRx input[name="va_far"]').val()
		);
		$("#idLeftRx input[name=va_int]").val(
			$('#idRightRx input[name="va_int"]').val()
		);
		$("#idLeftRx input[name=va_close]").val(
			$('#idRightRx input[name="va_close"]').val()
		);
		if (display == "") {
			display == "hide"
		...
		$(
			'#idRightRx input[name=rx_origin][value="' +
				$("#idLeftRx input[name=rx_origin]:checked").val() +
				'"]'
		).prop("checked", true);
		$("#idRightRx input[name=va_far]").val(
			$('#idLeftRx input[name="va_far"]').val()
...
for (let rx of idRxArr) {
	let arr;
	if (idRxArr.indexOf(rx) == 0) {
		// if right side ie first index
	...
	$(rx + " input[name=glass_type]").change(function () {
		if (
			$(rx + " input[name=glass_type]:checked").val() == "monofocal" ||
			$(rx + " input[name=glass_type]:checked").val() == "na"
		) {
	...
	$(rx + " input[name=rx_origin]").change(function () {
		if (
			$(rx + " input[name=rx_origin]:checked").val() == "trial" ||
			$(rx + " input[name=rx_origin]:checked").val() == "glass"
		) {
			...
			if (rx != "#rxFormModal") {
				...
				$(rx + " input[name=sph_int]").val(
					$(rx + " input[name=sph_far]").val()
				);
				$(rx + " input[name=cyl_int]").val(
					$(rx + " input[name=cyl_far]").val()
				);
				$(rx + " input[name=axis_int]").val(
					$(rx + " input[name=axis_far]").val()
				);
				$(rx + " input[name=sph_close]").val(
					$(rx + " input[name=sph_far]").val()
				);
				$(rx + " input[name=cyl_close]").val(
					$(rx + " input[name=cyl_far]").val()
				);
				$(rx + " input[name=axis_close]").val(
					$(rx + " input[name=axis_far]").val()
				);
...
$('#idRightRx input[name="timestamp"]').change(function () {
	$('#idLeftRx input[name="timestamp"]').val(
		$('#idRightRx input[name="timestamp"]').val()
	);
});
...
function syncRx() {
	...
	$("#btnLink").hasClass("unlocked")
		? $("#btnLink").removeClass("unlocked")
		: {};
	!$("#btnLink").hasClass("locked")
		? $("#btnLink").addClass("locked").html('<i class="fas fa-lock mx-1"></i>')
		: {};
	// console.log('locked!');
}
...
$("#btnLink").click(function () {
	...
	if ($("#btnLink").hasClass("locked") && !$("#btnLink").hasClass("unlocked")) {
		...
		$("#btnLink")
			.addClass("unlocked")
			.html('<i class="fas fa-unlock mx-1"></i>');
...
$('#idRightRx input[name="color"]').change(function () {
	$('#idLeftRx input[name="color"]').val(
		$('#idRightRx input[name="color"]').val()
	);
});
...
$("#rxFormModal").submit(function (e) {
	...
	crudp("rx", "0", req, dataStr).then((data) => {
		// Refresh only the relevant table based on laterality
		let laterality = $("#rxFormModal [name=laterality]").val();
		console.log("Modal laterality value:", laterality);
		console.log("dataObj laterality:", dataObj.laterality);
		console.log("typeof laterality:", typeof laterality);
		console.log("laterality === 'right':", laterality === "right");
		console.log("laterality === 'left':", laterality === "left");

		// Use the laterality from dataObj as it's more reliable
		let actualLaterality = dataObj.laterality || laterality;
		console.log("Using laterality:", actualLaterality);

		if (actualLaterality === "right") {
			console.log("Refreshing right table");
			$("#rxRight_tbl").bootstrapTable("refresh");
		} else if (actualLaterality === "left") {
			console.log("Refreshing left table");
			$("#rxLeft_tbl").bootstrapTable("refresh");
		} else {
			console.log("Fallback: refreshing all tables");
			refreshTables(tablesArr); // fallback with proper parameter
		}
	});
	$("#rxModal").modal("hide");
});
...
function refreshMachine(machine = "l80") {
	...
	$("#visionixRight_tbl").bootstrapTable("refresh", {
		url: API_R,
	});
	$("#visionixLeft_tbl").bootstrapTable("refresh", {
		url: API_L,
	});
}
...
function rxInsert(domId, laterality, id_pair, status = 1) {
	...
	dataStr = JSON.stringify(dataObj);
	crudp("rx", "0", "POST", dataStr).then((data) =>
...
function kmInsert(domId, laterality) {
	...
	dataObj["timestamp"] = new Date()
		.addHours(timeOffsetInHours)
		.toJSON()
		.slice(0, 16);
	console.log("dataObj", dataObj);
	dataStr = JSON.stringify(dataObj);
	crudp("km", "0", "POST", dataStr).then((data) =>
...
$.each(CV5000lst, function (i) {
	...
	$("#btnGetFrom" + CV5000lst[i]).click(function () {
		$("#cvRx_tbl").bootstrapTable("refresh", {
			url: API_CV5000 + "?machine=" + CV5000lst[i],
		});
		$("#cvKm_tbl").bootstrapTable("refresh", {
			url: API_CV5000 + "?machine=" + CV5000lst[i],
		});
		$("#importcvTitle").html(
			`Import from <strong>${CV5000lst[i].toUpperCase()}</strong>`
		);
	...
	$.ajax({
		...
		error: function (err) {
			...
			displayToast(
				"error",
				"Server error",
				"Can't reach server to export xml file",
				"3000"
			);
		},
	});
...
function exportDict2app(data) {
	statusDict = {
		...
		"Full Correction": "measure",
		"Latest prescription": "measure",
		"Current Spectacles": "measure",
		"Objective Data": "measure",
	};
	rx_originDict = {
		Prescription: "trial",
		"Full Correction": "autorx",
		"Latest prescription": "glass",
		"Current Spectacles": "glass",
		"Objective Data": "trial",
	};
	return;
}
...
function exportDictFormat(xdict) {
	...
	let kmLst = xdict["km"],
		rxLst = xdict["rx"],
		patientDict = xdict["patient"];
	dataObj["timestamp"] =
		getToday()["date"] + "T" + getToday()["time"].split(".")[0];
