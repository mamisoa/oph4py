			...
			success: function (data) {
				...
				if (data.status == "error" || data.count == 0) {
					...
					displayToast(
						"info",
						"GET phone request",
						"GET " + data.items[0].id_auth_user["username"],
						"6000"
					);
				}
			},
...
$("#btnEditmdParam").click(function () {
	getmdParam(id).then(function (data) {
...
function refreshList(listName) {
	if (listName == "userauth_user") {
		...
		userData.then(function (userData) {
			...
			$("#ulUserTitle").html("");
			$("#ulUserItems").html("");
			$("#userDetailsModal h5.modal-title").html(
				'Edit patient: <span class="fw-bold">' +
					checkIfDataIsNull(item.first_name) +
					" " +
					checkIfDataIsNull(item.last_name) +
					"</span>"
			);
			$("#userPhoneModal h5.modal-title").html(
				'New phone for: <span class="fw-bold">' +
					checkIfDataIsNull(item.first_name) +
					" " +
					checkIfDataIsNull(item.last_name) +
					"</span>"
			);
			$("#userAddressModal h5.modal-title").html(
				'New address for: <span class="fw-bold">' +
					checkIfDataIsNull(item.first_name) +
					" " +
					checkIfDataIsNull(item.last_name) +
					"</span>"
			...
			$("#patientTitle .patientName").html(
				checkIfDataIsNull(item.first_name, "n/a?") +
					" " +
					checkIfDataIsNull(item.last_name, "n/a?")
			);
			$("#patientTitle .patientDob").html(
				checkIfDataIsNull(item.dob, "n/a?") +
					" (" +
					getAge(checkIfDataIsNull(item.dob, "")) +
					" yo)"
			);
			$("#patientTitle .patientSSN").html(
				checkIfDataIsNull("NISS: " + item.ssn, "NISS: n/a?")
			);
			// fills list
			$("#ulUserItems").append(
				'<li class="list-group-item">Last name: <span class="text-uppercase fw-bold">' +
					checkIfDataIsNull(item.last_name) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Maiden name: <span class="text-uppercase fw-bold">' +
					checkIfDataIsNull(item.maiden_name) +
					"</span></li>"
			);
			$("#ulUserItems").append(
			...
			$("#ulUserItems").append(
				'<li class="list-group-item">Email: <span class="fw-bold">' +
					checkIfDataIsNull(item.email) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Membership: <span class="fw-bold">' +
					checkIfDataIsNull(item.membership["membership"]) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Date of birth: <span class="fw-bold">' +
					checkIfDataIsNull(item.dob) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Gender: <span class="fw-bold">' +
					checkIfDataIsNull(item.gender["sex"]) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">SSN: <span class="fw-bold">' +
					checkIfDataIsNull(item.ssn) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Marital status: <span class="fw-bold">' +
					checkIfDataIsNull(item["marital.marital_status"]) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Nationality: <span class="fw-bold">' +
					checkIfDataIsNull(item.nationality) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Country of birth: <span class="fw-bold">' +
					checkIfDataIsNull(item.birth_country) +
					"</span></li>"
			);
			$("#ulUserItems").append(
				'<li class="list-group-item">Notes: <span class="fw-bold">' +
					checkIfDataIsNull(item.user_notes) +
					"</span></li>"
			);
		});
		...
		userPhone.then(function (userData) {
			...
			for (const item of userData.items) {
				let listElement = "";
				listElement +=
					'<li class="list-group-item phone d-flex w-100 justify-content-between align-items-center" data-phone-id="' +
					item.id +
					'">';
				listElement +=
					'<span class="">' +
					checkIfDataIsNull(item.phone_origin) +
					': <span class="fw-bold">+' +
					checkIfDataIsNull(item.phone_prefix, "") +
					"-" +
				...
				listElement +=
					'<span class=""><button type="button" onclick="' +
					funcDel +
					'" class="btn btn-danger btn-sm m-2"><i class="fas fa-trash-alt"></i></button><button type="button" onclick="' +
					funcEdit +
					'" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></button></span></li>';
		...
		userAddress.then(function (userData) {
			for (const item of userData.items) {
				...
				let html =
					'<li class="list-group-item address" data-address-id="' +
					item.id +
					'">';
				html +=
					checkIfDataIsNull(item.address_origin, "") +
					' : <br><span class="fw-bold"> ' +
					checkIfDataIsNull(item.home_num, "") +
					" " +
					checkIfDataIsNull(item.box_num, "") +
					" ";
				html +=
					checkIfDataIsNull(item.address1, "") +
					" " +
					checkIfDataIsNull(item.address2, "") +
					"<br>" +
					checkIfDataIsNull(item.zipcode, "") +
					", " +
					checkIfDataIsNull(item.town, "") +
					"<br>" +
					checkIfDataIsNull(item.country, "") +
					"</span>";
				html +=
					'<span class="float-end"><button type="button" onclick="' +
					funcDel +
					'" class="btn btn-danger btn-sm me-2"><i class="fas fa-trash-alt"></i></button><button type="button" onclick="' +
					funcEdit +
					'" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></button></span>';
				html += "</li>";
		...
		mdParam.then(function (data) {
			if (data.count > 0) {
				let item = data.items[0];
				$("#mdParamItems").append(
					'<li class="list-group-item">Inami: <span class="fw-bold">' +
						checkIfDataIsNull(item.inami) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Professional email: <span class="fw-bold">' +
						checkIfDataIsNull(item.email) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office name: <span class="fw-bold">' +
						checkIfDataIsNull(item.officename) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office Address: <span class="fw-bold">' +
						checkIfDataIsNull(item.officeaddress) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office zip: <span class="fw-bold">' +
						checkIfDataIsNull(item.officezip) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office town: <span class="fw-bold">' +
						checkIfDataIsNull(item.officetown) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office country<span class="fw-bold">' +
						checkIfDataIsNull(item.officecountry) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office phone: <span class="fw-bold">' +
						checkIfDataIsNull(item.officephone) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Office url: <span class="fw-bold">' +
						checkIfDataIsNull(item.officeurl) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Company number: <span class="fw-bold">' +
						checkIfDataIsNull(item.companynum) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Company name: <span class="fw-bold">' +
						checkIfDataIsNull(item.companyname) +
						"</span></li>"
				);
				$("#mdParamItems").append(
					'<li class="list-group-item">Company IBAN: <span class="fw-bold">' +
						checkIfDataIsNull(item.companyiban) +
						"</span></li>"
				);
			}
		});
...
$("#btnEditUser").click(function () {
	$.ajax({
		url:
			...
			"/api/auth_user/" +
			id +
			"?@lookup=gender!:gender,marital!:marital,ethny!:ethny,membership!:membership",
		dataType: "json",
		type: "GET",
		success: function (data) {
			if (data.status != "error" || data.count != 0) {
				...
				document
					.getElementById("photob64")
					.setAttribute("src", checkIfDataIsNull(item.photob64, ""));
				document.getElementById("firstName").value = item.first_name;
				document.getElementById("lastName").value = item.last_name;
				document.getElementById("maidenName").value = checkIfDataIsNull(
					item.maiden_name,
					""
				);
				document.getElementById("email").value = item.email;
				document.getElementById("membershipSelect").value = checkIfDataIsNull(
					item["membership.id"],
					"6"
				);
				document.getElementById("genderSelect").value = checkIfDataIsNull(
					item["gender.id"],
					"Male"
				);
				document.getElementById("maritalSelect").value = checkIfDataIsNull(
					item["marital.id"],
					"1"
				);
				document.getElementById("dob").value = checkIfDataIsNull(item.dob, "");
				document.getElementById("nationality").value = checkIfDataIsNull(
					item.nationality,
					""
				);
				document.getElementById("birthCountry").value = checkIfDataIsNull(
					item.birth_country,
					""
				);
				document.getElementById("birthTown").value = checkIfDataIsNull(
					item.birth_town,
					""
				);
				document.getElementById("ethnySelect").value = checkIfDataIsNull(
					item["ethny.id"],
					"1"
				);
				document.getElementById("idcNum").value = checkIfDataIsNull(
					item.idc_num,
					""
				);
				document.getElementById("noblecondition").value = checkIfDataIsNull(
					item.noblecondition,
					""
				);
				document.getElementById("specialstatus").value = checkIfDataIsNull(
					item.specialstatus,
					""
				);
				document.getElementById("ssn").value = checkIfDataIsNull(item.ssn, "");
				document.getElementById("user_notes").value = checkIfDataIsNull(
					item.user_notes,
					""
				);
	...
	.addEventListener("click", async function (e) {
		try {
			...
			if (item["success"] == true) {
				...
				document.getElementById("ssn").value = checkIfDataIsNull(
					item.national_number,
					""
				);
				document
					.getElementById("photo")
					.setAttribute("src", "data:image/jpg;base64," + item.PHOTO_FILE);
				document.getElementById("photob64").value = checkIfDataIsNull(
					"data:image/jpg;base64," + item.PHOTO_FILE,
					""
				);
				...
				document.getElementById("box_num").value =
					addressArr[1] == undefined ? "" : addressArr[1];
				document.getElementById("address1").value = addressStr.split(
					addressArr[0]
				)[0];
				$("#userDetailsModal .photoDiv").removeClass("visually-hidden");
			} else {
...
$("#userAuth_userForm").submit(function (e) {
	...
	for (let [key, value] of Object.entries(formData)) {
		// to avoid resetting existing fields
		if (value == "") {
	...
	crudp("auth_user", id, "PUT", formData).then((data) =>
...
function userFormSubmit(table) {
	...
	if (formData["method" + capitalize(table) + "Submit"] == "PUT") {
		rec = formData["id" + capitalize(table) + "Submit"]; // save id for API URL
	}
	// Always remove id-related fields from payload
	delete formData["method" + capitalize(table) + "Submit"];
	delete formData["id" + capitalize(table) + "Submit"];
	// Don't add id to payload - it should only be in URL for PUT requests
	formData = JSON.stringify(formData); // change to string
	console.log(formData);
	crudp(table, rec, req, (data = formData)).then((data) =>
	...
	bootbox.confirm({
		...
		callback: function (result) {
			if (result == false) {
				...
				crudp(table, id, "DELETE").then((data) =>
					refreshList("user" + capitalize(table))
				);
...
function confirmEdit(recid, table) {
	...
	$.ajax({
		url:
			...
			"/" +
			recid +
			"?@lookup=name!:id_auth_user[first_name,last_name]",
		dataType: "json",
		type: "GET",
		success: function (data) {
			if (data.status != "error" || data.count != 0) {
				item = data.items[0];
				console.log(item.id);
				$("#user" + capitalize(table) + "Modal h5.modal-title").html(
					"Edit " +
						table +
						" for " +
				...
				if (table == "phone") {
					document.getElementById("originPhoneSelect").value =
						item.phone_origin;
					document.getElementById("phone_prefix").value = item.phone_prefix;
					document.getElementById("phone").value = item.phone;
				} else if (table == "address") {
					document.getElementById("originAddressSelect").value =
						item.address_origin;
				...
				document.getElementById("method" + capitalize(table) + "Submit").value =
					"PUT";
				document.getElementById("id" + capitalize(table) + "Submit").value =
					item.id;
			...
			success: function (data) {
				...
				if (data.status == "error" || data.count == 0) {
					displayToast(
						"error",
						"GET error",
						"Cannot retrieve medical registration infos",
						"6000"
					);
				} else {
					displayToast(
						"info",
						"GET request",
						"GET MD registration info" + data.items[0].id_auth_user["username"],
						"6000"
					);
				}
			},
...
$("#userMd_paramForm").submit(function (e) {
	...
	let rec = formData["id"] || "0"; // Save id for the API URL if it exists
	// Always remove id from payload regardless of method
	delete formData["id"];
	delete formData["methodmdParamSubmit"];
	for (let [key, value] of Object.entries(formData)) {
		// delete other empty keys to avoid resetting existing fields
		if (value == "") {
			console.log(key, ":empty");
			delete formData[key];
		}
	...
	crudp("md_params", rec, req, formStr).then((data) =>
