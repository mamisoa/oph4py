...
$("#userForm").submit(function (e) {
	...
	if (formObj["ssn"] != "") {
		...
		addressObj["address1"] =
			addressArr == null ? addressStr : addressStr.split(addressArr[0])[0];
...
function delUser(id, name) {
	bootbox.confirm({
		message:
			"Are you sure you want to delete this user: <strong>" +
			name +
			"</strong> ?",
		closeButton: false,
		buttons: {
...
// Function to sanitize text for username and email
function sanitizeForUsername(text) {
	// Remove accents using the existing norm function
	let sanitized = norm(text.toLowerCase());
	// Remove spaces and any characters that are not letters, numbers, or underscores
...
$("#userForm input[name=first_name]").change(function () {
	...
	let t = new Date();
	let firstName = $("#userForm input[name=first_name]").val();
	let sanitizedName = sanitizeForUsername(firstName);
	suffix =
		sanitizedName + parseInt(t.getMilliseconds()) + parseInt(t.getSeconds());
	...
	.addEventListener("click", async function (e) {
		try {
			...
			if (item["success"] == true) {
				...
				document.getElementById("newUserdob").value = checkIfDataIsNull(
					dob,
					""
				);
				document.getElementById("newUserNationality").value = checkIfDataIsNull(
					item.nationality,
					""
				);
				document.getElementById("newUserbirthTown").value = checkIfDataIsNull(
					item.location_of_birth,
					""
				);
				document.getElementById("newUserIdcNum").value = checkIfDataIsNull(
					item.card_number,
					""
				);
				document.getElementById("newUserSsn").value = checkIfDataIsNull(
					item.national_number,
					""
				);
				document
					.getElementById("photoidc")
					.setAttribute("src", "data:image/jpg;base64," + item.PHOTO_FILE);
				document.getElementById("newUserPhoto").value = checkIfDataIsNull(
					"data:image/jpg;base64," + item.PHOTO_FILE,
					""
				);
				document.getElementById("newUserAddress").value =
					item.address_street_and_number;
				document.getElementById("newUserZip").value = item.address_zip;
				document.getElementById("newUserTown").value =
