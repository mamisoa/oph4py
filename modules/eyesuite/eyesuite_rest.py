...
from ...settings import EYESUITE_FOLDER, EYESUITE_RESULTS_FOLDER, MACHINES_FOLDER
...
def createWlFile(device, id="", lastname="", firstname="", dob="", sex=""):
    ...
    if device not in ("PERIMETRY_STATIC", "BIOM_MEASUREMENT"):
        return {
            "result": "Invalid device value. Must be either 'PERIMETRY_STATIC' or 'BIOM_MEASUREMENT'."
        }
    if not id:
...
# find xml file
def find_matching_xml(
    date: str, lastname: str, firstname: str, ID: str = "", directory: str = "."
):
    """
    ...
    matching_files = [
        file for file in all_files if pattern.match(os.path.basename(file))
    ]
        ...
        if measurement_type in data:
            for measurement, value in data[measurement_type].items():
                if value is not None:
                    if measurement != "MODE":
                        if "," in value:
                            data[measurement_type][measurement] = float(
                                value.replace(",", ".")
                            )
                        else:
                            try:
    ...
    try:
        ...
        for filename, exams in data["exams"].items():
            ...
            for laterality, measures in exams.items():
                ...
                existing_exam = (
                    db(
                        (db.biometry.exam_date == exam_date)
                        & (db.biometry.id_auth_user == patient_id)
                        & (db.biometry.laterality == laterality)
                    )
                    .select()
                    .first()
                )
                ...
                db.biometry.insert(
                    ...
                    a_scan_central_cornea_thickness=measures["A-SCAN"][
                    ...
                    keratometry_flat_meridian_axis=measures["KERATOMETRY"][
                        "FLAT_MERIDIAN_AXIS"
                    ],
                    keratometry_steep_meridian=measures["KERATOMETRY"][
                        "STEEP_MERIDIAN"
                    ],
...
def upload_lenstar(wlId=None, id="", lastname="_", firstname="_"):
    ...
    filenames, params = find_matching_xml(
        directory=path,
        date=now,
        lastname=remove_diacritics(lastname),
        firstname=remove_diacritics(firstname),
        ID=id,
    )
    if len(filenames) == 0:
        return {
    ...
    for filename in filenames:
        ...
        examination_info = {
            child.tag: child.text for child in examination_node if child.tag == "DATE"
        }
...
def biometry(id_auth_user):
    ...
    for record in biometry_data:
        ...
        exams_data[datetimeKey][laterality] = {
            "A-SCAN": {
                "AQUEOUS_DEPTH": (
                    record["a_scan_aqueous_depth"]
                    if record["a_scan_aqueous_depth"]
                    else "N/A"
                ),
                "AXIAL_LENGTH": (
                    record["a_scan_axial_length"]
                    if record["a_scan_axial_length"]
                    else "N/A"
                ),
                "CENTRAL_CORNEA_THICKNESS": (
                    record["a_scan_central_cornea_thickness"]
                    if record["a_scan_central_cornea_thickness"]
                    else "N/A"
                ),
                "LENSE_THICKNESS": (
                    record["a_scan_lense_thickness"]
                    if record["a_scan_lense_thickness"]
                    else "N/A"
                ),
            ...
            "KERATOMETRY": {
                "FLAT_MERIDIAN": (
                    record["keratometry_flat_meridian"]
                    if record["keratometry_flat_meridian"]
                    else "N/A"
                ),
                "FLAT_MERIDIAN_AXIS": (
                    str(record["keratometry_flat_meridian_axis"])
                    if record["keratometry_flat_meridian_axis"]
                    else "N/A"
                ),
                "STEEP_MERIDIAN": (
                    record["keratometry_steep_meridian"]
                    if record["keratometry_steep_meridian"]
                    else "N/A"
                ),
            },
            "PUPILLOMETRY": {
                "BARYCENTER_X": (
                    record["pupillometry_barycenter_x"]
                    if record["pupillometry_barycenter_x"]
                    else "N/A"
                ),
                "BARYCENTER_Y": (
                    record["pupillometry_barycenter_y"]
                    if record["pupillometry_barycenter_y"]
                    else "N/A"
                ),
                "DIAMETER": (
                    record["pupillometry_diameter"]
                    if record["pupillometry_diameter"]
                    else "N/A"
                ),
            },
            "WHITE-WHITE": {
                "BARYCENTER_X": (
                    record["white_white_barycenter_x"]
                    if record["white_white_barycenter_x"]
                    else "N/A"
                ),
                "BARYCENTER_Y": (
                    record["white_white_barycenter_y"]
                    if record["white_white_barycenter_y"]
                    else "N/A"
                ),
                "DIAMETER": (
                    record["white_white_diameter"]
                    if record["white_white_diameter"]
                    else "N/A"
                ),
            },
        }
