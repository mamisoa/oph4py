...
def getWF(machine, path, filename, side, patient):
    ...
    try:
        ...
        [rx["sph3"], rx["cyl3"], rx["axis3"]] = [
            float(sph_dict["r_3"]),
            float(cyl_dict["r_3"]),
            float(axis_dict["r_3"]) * 180 / pi,
        ]
        [rx["sph5"], rx["cyl5"], rx["axis5"]] = [
            float(sph_dict["r_5"]),
            float(cyl_dict["r_5"]),
            float(axis_dict["r_5"]) * 180 / pi,
        ]
        [rx["sph7"], rx["cyl7"], rx["axis7"]] = [
            float(sph_dict["r_7"]),
            float(cyl_dict["r_7"]),
            float(axis_dict["r_7"]) * 180 / pi,
        ]
        rx["status"] = "success"
...
def getTopo(machine, path, filename, side, patient):
    ...
    try:
        ...
        [
            ...
            float(simk_dict["k1"]),
            float(simk_dict["k2"]),
            float(simk_dict["k1_axis"]),
            float(simk_dict["k2_axis"]),
            float(simk_dict["cyl"]),
            float(topo_dict["pd"]) / 2,
        ]
        topo["status"] = "success"
    except Exception as e:
        topo["error"] = str(e)
...
def getARK(machine, path, filename, side, patient):
    ...
    try:
        ...
        [rx["sph3"], rx["cyl3"], rx["axis3"]] = [
            float(ar["sphere_3"]),
            float(ar["cylinder_3"]),
            float(ar["axis_3"]) * 180 / pi,
        ]
        [rx["sph5"], rx["cyl5"], rx["axis5"]] = [
            float(ar["sphere_5"]),
            float(ar["cylinder_5"]),
            float(ar["axis_5"]) * 180 / pi,
        ]
        [rx["sph7"], rx["cyl7"], rx["axis7"]] = [
            float(ar["sphere_7"]),
            float(ar["cylinder_7"]),
            float(ar["axis_7"]) * 180 / pi,
        ]
        kr = dict(config.items("KR"))
        [rx["k1"], rx["k2"], rx["k1_axis"], rx["k2_axis"], rx["kcyl"]] = [
            float(kr["k1"]),
            float(kr["k2"]),
            kr["k1_axis"],
            kr["k2_axis"],
...
def addpatient_l80(firstname, lastname):
    ...
    try:
        ...
        return {
            "result": "error creating index:" + folder + "/" + filename + "/Index.txt"
        }
            ...
            with os.scandir(folder["path"]) as patientFolderitr:
                for examsFolders in patientFolderitr:  # datetime exam folders
                    if examsFolders.is_dir():
                        ...
                        dateFolderList[1] = month[
                            dateFolderList[1]
                        ]  # replace month str by num
                        yearTimeList = (dateFolderList.pop(-1)).split(
                            "#"
                        )  # extrude  and list year,time
                        yearTimeList[1] = ":".join(
                            yearTimeList[1].split("-")
                        )  # replace - by : in time
                        ...
                        with os.scandir(
                            examsFolders.path
                        ) as examFolderitr:  # topo wf folder
                                ...
                                if exam.is_dir():
                                    with os.scandir(exam.path) as examitr:
                                        for f in examitr:
                                            data = (
                                                []
                                            )  # contains all mesurements for a specific date
                                            if f.is_file():
                                                p = re.compile(
                                                    "(?P<side>Left|Right)(?P<exam>Topo|WF|ARK)_Meas_(?P<index>[0-9]+).txt"
                                                )
                                                m = p.search(f.name)  # get the file
                                                if m != None:
                                                    if (
                                                        m.group("side").lower() == side
                                                        or side == "both"
                                                    ):
                                                        patient["count"] += 1
                                                        if m.group("exam") == "WF":
