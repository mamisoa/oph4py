...
@action("test/files_bt", method=["GET"])
@action.uses(session, T, auth, db, "test/files_bt.html")
def test_files_bt():
    """
    Test controller for bootstrap-table with ES modules
    """
    env_status = ENV_STATUS
    timeOffset = TIMEOFFSET
    app_name = APP_NAME
    ...
    # This ensures the template has access to necessary variables
    return locals()
